...
class CSVWriter:
    ...
    def __init__(self, metadata: Metadata, path: str, s3_client: boto3.client = None):
        ...
        self.csv_converter = CSVConverter(metadata=metadata)
    ...
    def write(self, rows: int = None, desired_size: int = None, batch_size: int = 1000000, use_batches: bool = False,
        ...
        if use_batches:
            ...
            self._write_one_set(path=self.path, rows=rows_required, progress=progress)

    def _write_one_set(self, path: str, rows: int, progress: bool) -> None:
        """
        Writes a local csv file containing all data.
        """
        csv_data = self.csv_converter.create(rows=rows, progress=progress)
        self.dumper.write(path=f"{path}/{self.metadata.file_name}.csv", body=csv_data)

    def _write_in_batches(self, path: str, rows: int, batch_range: list, progress: bool) -> None:
        """
        ...
        for batch in batch_range:
            file_counter += 1
            row_counter += batch
            file_name = f"{self.metadata.file_name}-part_{file_counter}.csv"
            progress_msg = f"Creating part {file_counter} / {len(batch_range)} ({row_counter}/{rows})"

            csv_data = self.csv_converter.create(rows=batch, progress=progress, progress_message=progress_msg)
    ...
    def _estimate_rows_from_size(self, desired_size: float, sample_row_size: int = 100000, progress: bool = False):
        ...
        # Create the file
        csv_data = self.csv_converter.create(rows=sample_row_size, progress=progress,
